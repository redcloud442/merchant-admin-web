...
import { formatDateToYYYYMMDD } from "@/lib/function";
import { WithdrawListExportData } from "@/lib/types";
import { cn } from "@/lib/utils"; // optional utility for styling
import { getAdminWithdrawalExport } from "@/services/Withdrawal/Withdrawal";
      ...
      {exportData.length > 0 && (
        <CSVLink
          ...
          filename={`${selectedType}-${
            date.start ? formatDateToYYYYMMDD(date.start) : ""
          }-to-${
            date.end ? formatDateToYYYYMMDD(date.end) : ""
          }_withdrawals_export.csv`}
          ref={csvLinkRef}
          className="hidden"
        />
      )}
